    ...
    def serve(
        ...
        try:
            ...
            if not config.is_configured:
                console.print("[red]Error:[/red] Server not configured. Run 'pwndoc-mcp config init' first.")
                raise typer.Exit(1)
            
            if transport != "stdio":
                console.print(f"[green]Starting PwnDoc MCP Server[/green]")
...
if HAS_RICH:
    ...
    def config_show(
        ...
        table.add_row("Password", ("*" * 8 if config.password else "[dim]not set[/dim]") if not reveal_secrets else config.password)
        table.add_row("Token", ("*" * 20 if config.token else "[dim]not set[/dim]") if not reveal_secrets else (config.token or ""))
...
if HAS_RICH:
    ...
    def tools():
        ...
        for name, tool in sorted(server._tools.items()):
            table.add_row(name, tool.description[:60] + "..." if len(tool.description) > 60 else tool.description)
